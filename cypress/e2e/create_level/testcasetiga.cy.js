...
describe("CRUD Level di halaman /level", () => {
    ...
    // --- C R E A T E ---
    it("CREATE: Gagal menambahkan level baru jika nama level kurang dari 3 huruf", () => {
        cy.contains(/tambah level/i, { timeout: 10000 }).click();

        cy.get('input[name="level_kode"]').clear().type("DSN");
        cy.get('input[name="level_nama"]').clear().type("Do");
        ...
        cy.contains("td", "DSN").should("not.exist");
    });
