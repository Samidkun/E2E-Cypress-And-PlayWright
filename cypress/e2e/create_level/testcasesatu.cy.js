...
describe("CRUD Level di halaman /level", () => {
  ...
  // --- C R E A T E ---
  it("CREATE: Berhasil menambahkan level baru jika kode 3 huruf dan nama lebih dari 3 huruf", () => {
    cy.contains(/tambah level/i, { timeout: 10000 })
      .should("be.visible")
      .click();

    cy.get('input[name="level_kode"]').clear().type(LEVEL_KODE);
    cy.get('input[name="level_nama"]').clear().type(LEVEL_NAMA_LAMA);
    ...
    cy.contains("td", LEVEL_NAMA_LAMA, { timeout: 10000 }).should("exist");
  });

  // --- R E A D ---
  /*it("2. READ: Data level DSN muncul di tabel", () => {
    cy.contains("td", LEVEL_KODE, { timeout: 10000 }).should("exist");
  });*/
  ...
  // --- D E L E T E ---
  /*it("4. DELETE: Berhasil menghapus level DSN", () => {
    cy.get("table", { timeout: 10000 }).should("be.visible");
